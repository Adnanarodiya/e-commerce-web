...
import { DISCOUNT_MIN_SUBTOTAL, bulkDiscountPercent } from "@/lib/discounts";
      ...
      <CardContent className="space-y-4 text-start p-4 sm:p-6 pt-0 min-w-0" style={dirStyle}>
        ...
        <div className="space-y-3 min-w-0">
          ...
          {percentageDiscount > 0 && (
            <SummaryRow
              label={`${bulkPercent}% ${t("discount")} (books)`}
        ...
        {subtotal < DISCOUNT_MIN_SUBTOTAL && (
          <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg text-xs text-blue-800 leading-relaxed break-words">
            <p>
              {isRtl
                ? `₹5,000 یا اس سے زیادہ پر بینک %10 / کیش %15 اور قرآن پر ₹25/نقل — مزید ₹${(DISCOUNT_MIN_SUBTOTAL - subtotal).toFixed(2)} شامل کریں`
                : `Bank 10% / Cash 15% off books + ₹25/copy on Quran when you spend ₹${DISCOUNT_MIN_SUBTOTAL.toLocaleString()}+ — add ₹${(DISCOUNT_MIN_SUBTOTAL - subtotal).toFixed(2)} more`}
            </p>
          </div>
        )}
